...
def test_unable_update(client, create_user):
    """ Verify that authenticated non driver, non admin can't update trip with PATCH api/trips/<trip_id>/  """
...
def test_unable_delete(client, create_user):
    """ Verify that authenticated non driver, non admin can't delete trip with DELETE api/trips/<trip_id>/  """
    _, username, password = create_user
    client.login(username=username, password=password)
    trip = TripFactory()
    url = reverse("trips-detail", args=[trip.id])
...
def test_driver_delete(client, create_user):
    """ Verify that driver can delete it's own trip with DELETE api/trips/<trip_id>/ """
...
def test_driver_update(client, create_user):
    """ Verify that driver can delete it's own trip with PATCH api/trips/<trip_id>/ """
    pk, username, password = create_user
    user = User.objects.get(pk=pk)
    trip = TripFactory(driver=user)
    client.login(username=username, password=password)
