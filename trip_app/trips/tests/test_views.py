...
class TestTrips:
    ...
    def test_filter_trips_by_distance(self, admin_client, trips_with_different_coordinates):
        ...
        user_sp, user_dp = trips_with_different_coordinates[0].start_point, trips_with_different_coordinates[
            0].dest_point
    ...
    def test_filter_trips_by_time(self, admin_client, trips_with_different_dep_times):
    ...
    def test_reserve_trip(self, admin_client):
        """ Verify that user is able to reserve a trip with POST /api/trips/<trip_id>/reserve/
            and his name should appear in the list of passengers if man_approve field of a trip is
            False """
    ...
    def test_driver_reserve_trip(self, admin_client):
        """ Verify that driver of a trip gets 400 response to POST /api/trips/<trip_id>/reserve/ """
    ...
    def test_reserve_full_trip(self, admin_client):
        """ Verify that user cannot reserve a trip with no empty seats with POST /api/trips/<trip_id>/reserve/ """
        user = User.objects.create(username="test")
        # Trip with 1 seat and 1 passenger, meaning that there are no empty seats in it
    ...
    def test_reserve_trip_with_man_approve(self, admin_client):
        """ Verify that when user attempts to reserve a trip with manual approve
            via POST /api/trips/<trip_id>/reserve/ trip request is created and
            passenger list of a trip is still empty"""
    ...
    def test_trip_requests(self, admin_client):
        """ Verify that trip requests for a specific trip are returned for GET /api/trips/<trip_id>/resuests/ """
        client_user = User.objects.get(username="admin")
        trip = TripFactory()
